package com.clearcaptions.ccwsv3.common.boot.util;
...
public class ExternalFileLoaderUtil extends PropertiesLoaderUtils {
	...
	private static final PropertiesPropertySourceLoader propLoader = new PropertiesPropertySourceLoader();
	
	private static final YamlPropertySourceLoader ymlLoader = new YamlPropertySourceLoader();
	
	public static PropertySource<?> loadPropertySource(String prefix, String filename) throws IOException {
		...
		if(extension.contentEquals(PROPERTY_FILE_EXTENSION) || extension.contentEquals(XML_FILE_EXTENSION)) {
			return propLoader.load(prefix + fileNameWithoutExtension, path).get(0);
		} else if(extension.contentEquals(YML_FILE_EXTENSION) || extension.contentEquals(YAML_FILE_EXTENSION)) {
			return ymlLoader.load(prefix + fileNameWithoutExtension, path).get(0);
		} else {
			throw new IllegalArgumentException("Resource " + filename + " does not have a valid extension: " + extension);
		}
	}
	
	public static PropertySource<?> loadPropertySource(String sourceName, Resource file) throws IOException {
		...
		if(extension.contentEquals(PROPERTY_FILE_EXTENSION) || extension.contentEquals(XML_FILE_EXTENSION)) {
			...
			throw new IllegalArgumentException("Resource " + file + " does not have a valid extension: " + extension);
		}
	}
	
	public static PropertySource<?> loadYamlPropertySource(String sourceName, Resource file) throws IOException {
    	Assert.hasLength(sourceName, "String sourceName must not be null");
    	Assert.notNull(file, "Resource file must not be null");
    	Assert.isTrue(file.exists(), "Resource " + file + " does not exist");
		return ymlLoader.load(sourceName, file).get(0);
	...
	public static PropertySource<?> loadPropertiesPropertySource(String sourceName, Resource file) throws IOException {
    	Assert.hasLength(sourceName, "String sourceName must not be null");
    	Assert.notNull(file, "Resource file must not be null");
    	Assert.isTrue(file.exists(), "Resource " + file + " does not exist");
		return propLoader.load(sourceName, file).get(0);
